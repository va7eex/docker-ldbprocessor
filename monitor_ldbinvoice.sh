...
#        if [ "xls" = "${file##*.}" ]; then
		...
		python3 /usr/share/process_arinvoice.py /tmp/"${file%.*}.csv" $path$(date +%Y%m%d)"_for-PO-import-test.txt" $path$(date +%Y%m%d)"_pricedeltareport-test.txt" >> $path$(date +%Y%m%d)"_log_arinvoice.txt"
